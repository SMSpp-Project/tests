...
attaches to it every `:Solver` its `BlockSolverConfig` names (a
`MCFSolver< MCFSimplex >`, a `MCFSolver< RelaxIV >`, a `:MCFLemonSolver` and a
`:MILPSolver`), and cross-checks what they return while the instance changes:
costs, capacities and deficits, arcs opened and closed, arcs added and
deleted. What each change is, and which of them a run makes, is the bit-wise
code of its `-k`.
...
- [batch-small](batches/batch-small), the small instances of `MCFClassSolver`,
  static and with dynamic arcs: the fast one, and the one that walks every
