...
UC, so it is built as part of the `UC` folder in CMake, but it works on
CFL files exactly the same way.
...
- `-c` : solver configuration file. The configs live one folder up, for
  example `BSPar_CPLEX.txt` or `BSPar_HiGHS.txt`.
...
bash run_cfl_tests.sh --instances "cap102 cap121" --n "20 50" --k "5 10" \
    --methods "dupacova cssc" --solver BSPar_CPLEX.txt --seed 1
```
...
`cssc` is much slower than the other methods. It builds a full N by N cost
matrix, which means solving many optimization problems, and then solves a
partitioning problem on top of that. Use a small N when testing it, and
avoid running several instances of it at the same time on a machine with
limited memory.
