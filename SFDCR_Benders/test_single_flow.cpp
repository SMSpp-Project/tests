...
int main( int argc , char **argv )
{
 ...
 string fn = argv[ 1 ];
 int flux = std::stoi( argv[ 2 ] ) - 1;
 string f = argv[ 2 ];
 
 ofstream fout( fn.substr( 0 , fn.find_last_of( '.' ) ) + "_" + f + ".csv" ); 
  ...
  */
  ...
  dcrb_socp->apply( TestBlock_ben );
    ...
    */
        ...
        if( Ben_lb < 0 && ! dynamic_cast< SingleFlowDCRBendersSolver * >( ben )->is_DCR_feasible() )
        ...
        if( ( ( ( opt_val_pc - Ben_lb ) / opt_val_pc < -1e-3 || ( opt_val_pc - Ben_ub ) / opt_val_pc > 1e-3 ) )
          && ( ( opt_val_socp - Ben_lb ) / opt_val_socp < -1e-3 || ( opt_val_socp - Ben_ub ) / opt_val_socp > 1e-3 ) 
