...
int main( int argc , char **argv )
{
 ...
 fout<<"nflow,feasible,lb_SOCP_CPX,ub_SOCP_CPX,cpu_SOCP_CPX,lb_PC_CPX,ub_PC_CPX,cpu_PC_CPX,"
 ...
 for( int j = 0 ; j < min( 100 , NComm ) ; j++ ){  
  ...
  dcrb_socp->apply( TestBlock_ben );
    ...
    bool optimal_pc_GRB = true;
        ...
        c_start = std::clock();
        rtrn = socp_GRB->compute();
        lbound = socp_GRB->get_lb();
        ubound = socp_GRB->get_ub();
        ...
        if( Ben_lb < -1e-6 && ! dynamic_cast< SingleFlowDCRBendersSolver * >( ben )->is_DCR_feasible() )
        ...
        if( ( ( ( opt_val_pc - Ben_lb ) / opt_val_pc < -1e-2 || ( opt_val_pc - Ben_ub ) / opt_val_pc > 1e-2 ) )
          ...
          && abs( opt_val_socp_CPX - opt_val_pc_CPX ) / opt_val_socp_CPX < 1e-2 
          && abs( opt_val_socp_CPX - opt_val_socp_GRB ) / opt_val_socp_CPX < 1e-2 
          && abs( opt_val_socp_CPX - opt_val_pc_GRB ) / opt_val_socp_CPX < 1e-2
          && feasible ){
